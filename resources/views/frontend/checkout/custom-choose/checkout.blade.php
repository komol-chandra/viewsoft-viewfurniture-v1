...
@section('title', 'Custom Product Checkout Page')
@section('css')
<style>
    .form-custom {
        margin-top: 10px;
...
<section class="section-tb-padding">
    <form method="post" action="{{ route('custom-choose-checkout.save') }}">@csrf
        <div class="container">
            <div class="row">
                <div class="col">
                    <div class="checkout-area">
                        <div class="billing-area">
                            <div style="padding: 20px">
                                ...
                                <div class="billing-form">
                                    ...
                                    @if ($data->custom_product_price > 500)
                                    <ul class="billing-ul" id="">
                                        <li class="billing-li"
                                            style="margin-top: 10px; margin-bottom: 10px ;margin-left: 20px;">
                        ...
                        <input type="hidden" name="request_id" id="request_id" value="{{ $id }}">
                        <div class="order-area" id="custom_checout_cart">
...
<input type="hidden" value="{{ $data->custom_product_price }}" id="cart_total_amount">
...
<script>
    function mainCheckoutCustomCart() {
        let request_id = $('#request_id').val();
        $.ajax({
            url: "{{url('/dashboard/custom-choose-checkout/page/')}}/" + request_id,
            type: 'get',
            success: function (data) {
                $("#custom_checout_cart").html(data);
            }
        });
    }
    mainCheckoutCustomCart();
...
<script>
    function alltotal() {
        ...
        if (cupon_amount > 0 && cupon_type == 'taka') {
            qty_price = qty_price - cupon_amount;
            ...
            let cuppon_discount = (qty_price * cupon_amount / 100);
            qty_price = qty_price - cuppon_discount;
        ...
        if (city_id == 47) {
            ...
            var shopping_charge_amount = parseFloat(cart_total_amount) * parseFloat((dhaka_charge / 100));
            $('#shipping_percent').text("% " + dhaka_charge);
            $('#shopping_amount').text("৳ " + shopping_charge_amount);
            var grand_total = parseFloat(shopping_charge_amount) + parseFloat(qty_price);
            ...
            var shopping_charge_amount = parseFloat(cart_total_amount) * parseFloat((out_of_dhaka_charge / 100));
